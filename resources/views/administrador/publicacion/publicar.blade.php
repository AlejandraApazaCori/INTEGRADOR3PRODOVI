                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="p-4 rounded-xl border" style="background: linear-gradient(135deg, #eff6ff, #f5f3ff); border-color: #bfdbfe;">
                                    ...
                                    <div id="optimization-panel" data-endpoint="{{ route('administrador.publicaciones.horarios', ['tarea_id' => $tarea->id]) }}" class="hidden mt-4 pt-4" style="border-top: 1px solid rgba(79,70,229,0.12);">
                                        <div class="rp-optimization-panel">
                                            <div class="rp-optimization-header">
                                                <div class="rp-optimization-header__icon">
                                                    <i class="fas fa-chart-bar"></i>
                                                </div>
                                                <div class="rp-optimization-header__copy">
                                                    <span>Recomendación inteligente</span>
                                                    <h3>Mejores horarios para publicar</h3>
                                                    <p><i class="far fa-calendar-alt"></i> Próximos 7 días <b>·</b> Zona horaria: America/La_Paz</p>
                                                </div>
                                            </div>
                                            <div class="rp-optimization-explainer">
                                                <i class="fas fa-info-circle"></i>
                                                <p><strong>¿Cómo leerlo?</strong> Un puntaje más alto indica mayor interacción estimada. Se calcula con reacciones/me gusta + 2 × comentarios. El horario elegido se aplicará a todas las redes seleccionadas.</p>
                                            </div>
                                            ...
                                            <button type="button" id="optimization-refresh" class="rp-optimization-refresh">
                                                <i class="fas fa-sync-alt"></i> Actualizar estimaciones
                                            </button>
                                        </div>
                                    </div>
                                </div>
...
<style>
    ...
    .rp-optimization-panel{overflow:hidden;border:1px solid #d9e7e8;border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(17,126,140,.09)}
    .rp-optimization-header{display:flex;align-items:center;gap:13px;padding:18px 19px;background:linear-gradient(135deg,#0f7580,#1594a2);color:#fff}
    .rp-optimization-header__icon{width:43px;height:43px;display:grid;place-items:center;flex:0 0 43px;border:1px solid rgba(255,255,255,.24);border-radius:12px;background:rgba(255,255,255,.15);font-size:1rem}
    .rp-optimization-header__copy>span{display:block;margin-bottom:2px;color:#c8f2f5;font-size:.52rem;font-weight:900;letter-spacing:.12em;text-transform:uppercase}
    .rp-optimization-header__copy h3{margin:0;color:#fff;font-size:1rem;font-weight:900;letter-spacing:-.015em}
    .rp-optimization-header__copy p{margin:4px 0 0;color:#e1f6f8;font-size:.64rem}.rp-optimization-header__copy p i{margin-right:4px}.rp-optimization-header__copy p b{padding:0 3px}
    .rp-optimization-explainer{display:flex;align-items:flex-start;gap:9px;margin:14px 15px 4px;padding:11px 12px;border:1px solid #e5edd8;border-radius:10px;background:#f7faF3;color:#58634f;font-size:.65rem;line-height:1.5}
    .rp-optimization-explainer>i{margin-top:2px;color:#7da533}.rp-optimization-explainer p{margin:0}.rp-optimization-explainer strong{color:#3d4935}
    .publication-page #optimization-status{margin:10px 16px 0;font-size:.66rem!important;line-height:1.45}
    .publication-page #optimization-results{padding:6px 15px 2px}
    .rp-prediction-card{overflow:hidden;border:1px solid #e1e8df!important;border-radius:13px!important;background:#fff;box-shadow:0 5px 15px rgba(48,40,52,.045)}
    .rp-prediction-card__head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:13px 14px;border-bottom:1px solid #edf0eb;background:#fafbf9}
    .rp-prediction-account{display:flex;align-items:center;gap:9px;min-width:0}.rp-prediction-account__icon{width:34px;height:34px;display:grid;place-items:center;flex:0 0 34px;border-radius:10px;color:#fff;font-size:.9rem}.rp-prediction-account__icon.is-instagram{background:linear-gradient(135deg,#833ab4,#e1306c,#f77737)}.rp-prediction-account__icon.is-facebook{background:#1877f2}
    .rp-prediction-account small{display:block;color:#8a9286;font-size:.49rem;font-weight:900;letter-spacing:.09em;text-transform:uppercase}.rp-prediction-account h4{overflow:hidden;margin:2px 0 0;color:#343a31;font-size:.75rem;font-weight:900;text-overflow:ellipsis;white-space:nowrap}
    .rp-prediction-badge{flex:0 0 auto;padding:5px 8px;border-radius:999px;background:#edf5e5;color:#638522;font-size:.51rem;font-weight:900}
    .rp-prediction-meta{display:flex;flex-wrap:wrap;gap:6px 12px;padding:10px 14px;color:#737b70;font-size:.59rem}.rp-prediction-meta span{display:inline-flex;align-items:center;gap:5px}.rp-prediction-meta i{color:#117e8c}
    .rp-prediction-note{display:flex;gap:7px;margin:0 14px 11px;padding:9px 10px;border-radius:9px;background:#f4f7f8;color:#667173;font-size:.59rem;line-height:1.45}.rp-prediction-note i{margin-top:2px;color:#117e8c}
    .rp-prediction-title{margin:3px 14px 8px;color:#393f36;font-size:.63rem;font-weight:900}
    .rp-prediction-options{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:7px;padding:0 14px 14px}
    .rp-prediction-option{position:relative;min-width:0;padding:9px 7px;border:1px solid #dfe8dd;border-radius:10px;background:#fff;color:#596054;text-align:left;transition:.18s}.rp-prediction-option:hover{transform:translateY(-2px);border-color:#117e8c;background:#f2fafb;box-shadow:0 7px 14px rgba(17,126,140,.12)}.rp-prediction-option.is-best{border-color:#9fc6ca;background:linear-gradient(145deg,#eef9fa,#fff)}
    .rp-prediction-option__rank{display:block;color:#8b9388;font-size:.47rem;font-weight:900;text-transform:uppercase}.rp-prediction-option__date{display:block;margin-top:3px;color:#363c34;font-size:.59rem;font-weight:900;line-height:1.3}.rp-prediction-option__score{display:block;margin-top:6px;color:#117e8c;font-size:.8rem;font-weight:900}.rp-prediction-option__score small{font-size:.48rem}.rp-prediction-option__samples{display:block;margin-top:2px;color:#91988e;font-size:.48rem;line-height:1.3}.rp-prediction-option__action{display:block;margin-top:7px;color:#117e8c;font-size:.49rem;font-weight:900}.rp-prediction-option.is-best:before{content:'Mejor';position:absolute;top:-7px;right:5px;padding:2px 5px;border-radius:999px;background:#ef6c22;color:#fff;font-size:.43rem;font-weight:900;text-transform:uppercase}
    .rp-prediction-chart{height:260px;margin:0 14px 14px;padding:12px;border:1px solid #edf0eb;border-radius:11px;background:#fcfdfb}.rp-prediction-chart__title{margin:0 0 8px;color:#697166;font-size:.55rem;font-weight:900;text-transform:uppercase}
    .rp-prediction-history{margin:0 14px 13px;padding:9px 10px;border-left:3px solid #7da533;border-radius:6px;background:#f6f9f2;color:#687263;font-size:.56rem;line-height:1.45}
    .rp-optimization-refresh{display:inline-flex;align-items:center;gap:6px;margin:9px 15px 15px;padding:8px 11px;border:1px solid #d4e2e3;border-radius:8px;background:#fff;color:#117e8c;font-size:.58rem;font-weight:900;transition:.18s}.rp-optimization-refresh:hover{border-color:#117e8c;background:#eef8f9}.rp-optimization-refresh:disabled{cursor:wait;opacity:.55}
    ...
    @media(max-width:760px){.rp-prediction-options{grid-template-columns:repeat(2,minmax(0,1fr))}.rp-prediction-options .rp-prediction-option:last-child{grid-column:1/-1}.rp-prediction-card__head{align-items:flex-start;flex-direction:column}.rp-prediction-chart{height:235px}}
    @media(max-width:640px){.publication-context,.publication-page .rp-card{width:calc(100% - 24px)}.publication-context{grid-template-columns:1fr}.publication-hero-body{padding:26px 20px}.publication-step>label:after,.publication-step>div>label:after,.publication-preview-column>label:after{margin-top:6px}.publication-page .rp-publish-btn{min-width:0}.publication-meta-modal{padding:12px}.publication-meta-dialog{padding:25px 18px}.publication-meta-dialog footer{grid-template-columns:1fr}.rp-optimization-header{align-items:flex-start;padding:15px}.rp-optimization-header__copy h3{font-size:.9rem}.rp-prediction-options{grid-template-columns:1fr}.rp-prediction-options .rp-prediction-option:last-child{grid-column:auto}.rp-prediction-option{display:grid;grid-template-columns:1fr auto;column-gap:10px}.rp-prediction-option__score{grid-column:2;grid-row:1/3;margin:0;align-self:center}.rp-prediction-option__samples,.rp-prediction-option__action{grid-column:1/-1}.rp-prediction-chart{height:250px}}
</style>
