@php
    ...
    $estadosTarea = [
        'no_iniciado' => ['titulo' => 'No iniciado', 'icono' => 'fa-circle'],
        'pendiente' => ['titulo' => 'Pendiente', 'icono' => 'fa-clock'],
        'en_curso' => ['titulo' => 'En curso', 'icono' => 'fa-spinner'],
        'entregado' => ['titulo' => 'Entregado', 'icono' => 'fa-box'],
        'reformular' => ['titulo' => 'Reformular', 'icono' => 'fa-rotate-left'],
        'aprobado' => ['titulo' => 'Aprobado', 'icono' => 'fa-circle-check'],
        'publicado' => ['titulo' => 'Publicado', 'icono' => 'fa-bullhorn'],
    ];
@endphp
...
<section class="tasks-workspace" data-task-workspace>
    ...
    <div class="tasks-board" data-task-panel="board">
        @foreach($estadosTarea as $estado => $configuracion)
            @php $tareasColumna = $tareas->where('estado', $estado); @endphp
    ...
    <div class="tasks-table-view is-hidden" data-task-panel="table">
        <div class="tasks-table-filters" role="group" aria-label="Filtrar tareas por estado">
            ...
            @foreach($estadosTarea as $estado => $configuracion)
                <button type="button" data-task-filter="{{ $estado }}"><i class="fas {{ $configuracion['icono'] }}"></i> {{ $configuracion['titulo'] }} <span>{{ $tareas->where('estado', $estado)->count() }}</span></button>
            @endforeach
        </div>
                ...
                <table class="tasks-table">
                    ...
                    <tbody>
                        @foreach($tareas as $tarea)
                            @php
                                $estadoClase = match($tarea->estado) {'no_iniciado', 'pendiente' => 'is-pending', 'en_curso' => 'is-progress', 'entregado', 'aprobado', 'publicado' => 'is-complete', default => 'is-cancelled'};
                            ...
                            @endphp
                            <tr data-task-row data-task-id="{{ $tarea->id }}" data-task-status="{{ $tarea->estado }}">
...
<style>
    ...
    @media(max-width:1000px){.tasks-board{grid-template-columns:repeat(7,minmax(245px,1fr))}.task-column-body{min-height:150px}}
...
function initializeTaskWorkspace() {
    ...
    const statusNames = { no_iniciado: 'No iniciado', pendiente: 'Pendiente', en_curso: 'En curso', entregado: 'Entregado', reformular: 'Reformular', aprobado: 'Aprobado', publicado: 'Publicado' };
    const statusClasses = { no_iniciado: 'is-pending', pendiente: 'is-pending', en_curso: 'is-progress', entregado: 'is-complete', reformular: 'is-cancelled', aprobado: 'is-complete', publicado: 'is-complete' };
    ...
    function syncPublishAction(card, status) {
        const currentAction = card.querySelector('.task-card-publish');
        if (status !== 'aprobado') {
        ...
        action.href = card.dataset.publishUrl;
        action.className = 'task-card-publish';
        const rocket = document.createElement('i');
        rocket.className = 'fas fa-rocket';
        const label = document.createElement('span');
        label.textContent = 'Publicar contenido';
        const arrow = document.createElement('i');
        arrow.className = 'fas fa-arrow-right';
        action.append(rocket, label, arrow);
        card.querySelector('.task-card-actions').before(action);
    }
    async function persistStatus(card, status) {
        ...
        if (statusLabel) statusLabel.textContent = statusNames[status];
        card.querySelectorAll('[data-status-option]').forEach(option => option.classList.toggle('is-selected', option.dataset.statusOption === status));
        syncPublishAction(card, status);
    ...
    workspace.querySelectorAll('.task-kanban-card').forEach(card => {
        ...
        const trigger = card.querySelector('[data-status-trigger]');
        const menu = card.querySelector('[data-status-menu]');
        trigger?.addEventListener('click', event => {
            event.stopPropagation();
            workspace.querySelectorAll('[data-status-menu]').forEach(otherMenu => { if (otherMenu !== menu) { otherMenu.hidden = true; otherMenu.closest('[data-status-dropdown]')?.querySelector('[data-status-trigger]')?.setAttribute('aria-expanded', 'false'); } });
            menu.hidden = !menu.hidden;
            trigger.setAttribute('aria-expanded', menu.hidden ? 'false' : 'true');
        });
        card.querySelectorAll('[data-status-option]').forEach(option => option.addEventListener('click', async event => {
            event.stopPropagation();
            ...
            catch (error) { previousZone.insertBefore(card, previousZone.querySelector('[data-column-empty]')); card.dataset.taskStatus = previousStatus; card.querySelector('[data-status-label]').textContent = statusNames[previousStatus]; refreshCounts(); showMessage('No se pudo mover la tarea. Inténtalo nuevamente.', true); }
        }));
    });
    ...
    document.addEventListener('click', () => workspace.querySelectorAll('[data-status-menu]').forEach(menu => { menu.hidden = true; menu.closest('[data-status-dropdown]')?.querySelector('[data-status-trigger]')?.setAttribute('aria-expanded', 'false'); }));
